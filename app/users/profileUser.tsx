...
export default function UserProfile() {
  ...
  const [editedBirthDate, setEditedBirthDate] =
    useState('');
  ...
  const handleDateChange = (text: string) => {
    ...
    if (length <= 2) {
      ...
      setEditedBirthDate(
        `${cleaned.slice(0, 2)}-${cleaned.slice(
          2
        )}`
      );
    } else {
          ...
          onPress: async () => {
            try {
              ...
              if (
                ...
                Alert.alert(
                  'Seguridad',
                  'Para eliminar tu cuenta, debes cerrar sesión e iniciarla nuevamente por seguridad.'
                );
              } else {
                Alert.alert(
                  'Error',
